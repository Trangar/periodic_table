...
").unwrap();
    ...
    let mut reader = csv::Reader::from_file("data.csv").unwrap().has_headers(true);
    let headers: Vec<String> = reader.headers().unwrap();
    let headers = headers.into_iter().map(underscore_case).collect::<Vec<String>>();
    for record in reader.decode() {
        ...
        items[6] = if items[6].len() == 0 { String::from("None") } else { format!("Some({}_f32)", items[6]) };
        // pub atomic_radius: Option<u32>,
        items[7] = if items[7].len() == 0 { String::from("None") } else { format!("Some({})", items[7]) };
        // pub ion_radius: Option<IonRadius>,
        items[8] = if items[8].len() == 0 { String::from("None") } else {
        ...
        items[9] = if items[9].len() == 0 { String::from("None") } else { format!("Some({})", items[9]) };
        // pub ionization_energy: Option<u32>,
        items[10] = if items[10].len() == 0 { String::from("None") } else { format!("Some({})", items[10]) };
        // pub electron_affinity: Option<i32>,
        items[11] = if items[11].len() == 0 { String::from("None") } else { format!("Some({})", items[11]) };
        ...
        items[15] = if items[15].len() == 0 { String::from("None") } else { format!("Some({})", items[15]) };
        // pub boiling_point: Option<u32>,
        items[16] = if items[16].len() == 0 { String::from("None") } else { format!("Some({})", items[16]) };
        // pub density: Option<f32>,
        items[17] = if items[17].len() == 0 { String::from("None") } else { format!("Some({}_f32)", items[17]) };
...
fn uppercase(string: &str) -> String{
    format!("{}{}", string.chars().next().unwrap().to_uppercase(), &string[1..])
}